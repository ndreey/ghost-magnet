...
#SBATCH --job-name=metasens_megahit     # name that will show up in the queue
#SBATCH --array=1-11%4
#SBATCH --output=slurm-%j.out             # filename of the output; the %j is equal to jobID
#SBATCH --error=slurm-%j.err              #
#SBATCH --partition=cpuqueue              #
#SBATCH --ntasks=1                        # number of tasks (analyses) to run
#SBATCH --cpus-per-task=8               # the number of threads allocated to each task
...
megahit -t 4 --presets meta-sensitive \
    -1 ${reads}/${hc_prefix}_trim_R1.fq.gz \
    -2 ${reads}/${hc_prefix}_trim_R2.fq.gz \
    -o meta_sens/${hc_prefix} \
    --out-prefix "${hc_prefix}_sensi"
